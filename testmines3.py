...
mines={ 0:{'value':0,'block':True},
        ...
        18:{'value':0,'block':False},
        19:{'value':0,'block':True},
        20:{'value':0,'block':True},
        21:{'value':0,'block':False}
      }
    ...
    def hit(self):
        ...
        if (self.attributes.has_key('hp')):
            ...
            if(self.maskSet):
                ...
                self.maskImg=self.maskSet[maskState]
       ...
       nextPos[Y]>=0 and nextPos[Y]<=tilemap.getSize()[Y]):
        ...
        if(nextTile.getAttributes()['block']):
            ...
            if(player.doAction(1,(move[X]*tilemap.getTileSize()[X],move[Y]*tilemap.getTileSize()[Y]))):
                return move #this isn't really needed...just kind of makes sense

def scrollMap(screen,player,tilemap,moveVect):
    if (player.getPos()[X]<4 or player.getPos()[X]>tilemap.getSize()[X]-4):
...
def game(screen):
    ...
    stats = {'sp':1,'str':1}
    player = Miner((4,4),SpriteSet(spriteImg,(48,48),spriteTemplate),stats)
    ...
    while True:
        ...
        #done hitting
            ...
            if(hitResult):
                ...
                hitTile.img = spriteset[MINE_DUG]
