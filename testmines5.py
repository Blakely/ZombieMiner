...
class MineMap(list):
    def __init__(self, template, tileSet, tileSize, player=None,tileMaskSet=None):
        ...
        self.mobs = dict() #init dict for mobs
    ...
    def clearMob(self,name):
        if(self.mobs.has_key(name)):
            self.mobs[name]=None
    
    def addMobs(self,mobs):
        for mob in dict(mobs).keys():
            self.mobs[mob] = mobs[mob]
    
    #def addMob(self,name,mob):
    #   self.mobs[name]=mob
    ...
    def draw(self,screen):
        ...
        for mob in dict(self.mobs).values():
            #make sure mob is visible (in screen area) before drawing it
            if (mob.getPos()[X]>=-self.tileSize[X] and mob.getPos()[X]<=screen.get_width()):
...
# screen - 
def game(screen):
    #load the spriteset image for the player (miner) and zombies
    playerImg=loadImage(IMG_PLAYER,TILE_TRANSCOLOR)
    zombieImg=loadImage(IMG_ZOMBIE,TILE_TRANSCOLOR)
    ...
    #setup zombie stats and create the zombie
    zombieStats = {'sp':0.2,'str':0.2}
    zombie = Mob((5,4),SpriteSet(zombieImg,(48,48),spriteTemplate),zombieStats)
    ...
    #create the MineMap (tilemap for the game) and add zombies
    tilemap=MineMap(template,tileset,(48,48),player,maskSet)
    tilemap.mobs["zombie"]=zombie
