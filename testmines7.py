...
class Miner(Drawable):
    ...
    def animateMask(self,elapsed,delay):
        if (elapsed - self.lastMaskMod > delay):
            self.maskFrame += 1
            if self.maskFrame >= len(self.maskSet):
                self.maskFrame = 0
                return True
            self.maskImg = self.maskSet[maskFrame]
            self.lastMaskMod=elapsed
        return False     
    
    def updateMask(self,maskSet,delay):
        #if this is a new maskSet, store it
        if(self.maskSet!=maskSet):
            self.maskSet=maskSet
        
        #try to animate the mask - when its done, return success   
        if (self.animateMask(pygame.time.get_ticks(),delay)):
            self.maskImg=None
            return True
...
class AI(object):
    ...
    def chooseDir(self,myPos,nearTiles):
        ...
        return DIR_UP #default to trying to go up - doesn't matter, AI cant move
...
def game(screen):
    ...
    template.setBorder(MINE_BLOCK) #set the border to be all unbreakable bricks
    ...
    while True:
        ...
        for zombie in zombies: 
            ...
            if (tilemap.getTile(zombie.getPos()).attributes['block']==MINE_BLOCK_FULL):
                zombie.updateMask(fireSet,10)
            
            #otherwise, let the AI kick in to choose a direction,then try and act in that direction
            else: 
