...
MINE_BLOCK_UP=1
MINE_BLOCK_NONE = 0
MINE_DIGGABLE = MINE_BLOCK_NONE #just for readability
MINE_SHOP=3

MINE_VAL_WIN=-1

mines={ #diggable tiles - first is blank, last is win, others are minable
        MINE_BLANK: {'chance':80,'value':1,'type':MINE_DIGGABLE,'hits':4},
        2:          {'chance':22,'value':1,'type':MINE_DIGGABLE,'hits':5},
        3:          {'chance':17,'value':1,'type':MINE_DIGGABLE,'hits':7},
        4:          {'chance':15,'value':1,'type':MINE_DIGGABLE,'hits':8},
        5:          {'chance':14,'value':1,'type':MINE_DIGGABLE,'hits':10},
        6:          {'chance':12,'value':1,'type':MINE_DIGGABLE,'hits':12},
        7:          {'chance':10,'value':1,'type':MINE_DIGGABLE,'hits':15},
        8:          {'chance':9,'value':1,'type':MINE_DIGGABLE,'hits':17},
        9:          {'chance':8,'value':1,'type':MINE_DIGGABLE,'hits':20},
        10:         {'chance':7,'value':1,'type':MINE_DIGGABLE,'hits':23},
        11:         {'chance':6,'value':1,'type':MINE_DIGGABLE,'hits':26},
        12:         {'chance':5,'value':1,'type':MINE_DIGGABLE,'hits':30},
        13:         {'chance':4,'value':1,'type':MINE_DIGGABLE,'hits':32},
        14:         {'chance':3,'value':1,'type':MINE_DIGGABLE,'hits':34},
        15:         {'chance':2,'value':1,'type':MINE_DIGGABLE,'hits':35},
        16:         {'chance':1,'value':1,'type':MINE_DIGGABLE,'hits':40},
        MINE_WIN:   {'chance':0,'value':MINE_VAL_WIN,'type':MINE_DIGGABLE,'hits':80},
...
ALIGN_CENTER=-1
WIN_TITLE_SIZE = 50
WIN_TITLE_FONT = pygame.font.Font(None,WIN_TITLE_SIZE)
WIN_FONT_SIZE = 25
WIN_FONT = pygame.font.Font(None,WIN_FONT_SIZE)
WIN_FONT_COLOR = pygame.Color(158,100,74)
...
COST_STR=20
COST_SP=20
SHOP_TITLE="Ye Olde Shop"
SHOP_PANELS={1:{'text':"Strength  $" + str(COST_STR)},
             2:{'text':"Speed     $" + str(COST_SP)}}
    
def centerPos(pos,bSize,sSize):
    #if the x position is meant to be "centered", center it
    if (pos[X]==ALIGN_CENTER):
        newPos = (bSize[X]/2.0 - sSize[X]/2.0,pos[Y]) #force float divison
    else:
        newPos = pos
    
    #if the y pos is meant to be "centered", center it
    if (pos[Y]==ALIGN_CENTER):
        newPos = (newPos[X],bSize[Y]/2.0 - sSize[Y]/2.0) #force float divison
        
    return newPos;        

class Button(object):
    def __init__(self,pos,btnSet,textImg,func=None):
        self.pos = pos
        self.func = func
        ...
        drawPos=(0,0)
        self.img.blit(btnSet[BTN_IMG_LEFT],drawPos) #draw left border of button

        #draw middle portion of button
        for w in range(0,textWidth):
            #+1 to accoutn for left border img
            drawPos=((1+w)*pnlSize[X],0)
            self.img.blit(btnSet[BTN_IMG_MID],drawPos)
        
        self.img.blit(btnSet[BTN_IMG_RIGHT],(self.size[X]-pnlSize[X],0)) #draw right border of button
    ...
    def click(self,pos):
        if (self.pos[0]<pos[0] and self.pos[0]+self.size[0]>pos[0]):
            if (self.pos[1]<pos[1] and self.pos[1]+self.size[1]>pos[1]):
                if (self.func):
                    self.func()
                return True;
...
class Label(object):
    def __init__(self,text,font,color,relPos,transColor=None):
        self.pos=relPos
        self.color=color
        self.font=font
        self.text=text
        self.transColor=transColor
        self.img=textImage(text,font,color,transColor)
    ...
    def change(self,text=None,font=None,color=None):
        ...
        self.img=textImage(self.text,self.font,self.color,self.transColor)
...
class Window(object):
    def __init__(self,pos,winSet,numPanels,title=None,labels=None,btns=None):
        ...
        #create the panels for the window
        self.createPanels(numPanels)
        
    def createPanels(self,numPanels):
        self.panels=list()
        self.panels.append(self.winSet[WIN_IMG_TOP]) #add top panel (window border)
        
        if(self.title): #add an extra panel if theres a title
            self.title=textImage(self.title,WIN_TITLE_FONT,WIN_FONT_COLOR)
            self.panels.append(self.winSet[WIN_IMG_PNL])
        
        #add middle panels
        for p in range(0,numPanels):
            self.panels.append(self.winSet[WIN_IMG_PNL])
            
        self.panels.append(self.winSet[WIN_IMG_BTM]) #add bottom panel (window border)
        
    def draw(self,screen):
        #center the position of the window if need be
        drawPos=centerPos(self.pos,screen.get_size(),self.pnlSize)
        ...
        if (self.labels):
            for label in self.labels:
                label.draw(screen,drawPos)

        #draw all the buttons
        if(self.btns):
            for btn in self.btns:
                btn.draw(screen,drawPos)
...
def game(screen):
    ...
    btnText = textImage("Buy",WIN_FONT,WIN_FONT_COLOR)
    btnSet=TileSet('buttonSet.png',(20,20),TILE_TRANSCOLOR)
    btn = Button((240,40),btnSet,btnText)
    
    winSet = TileSet('winSet.png',(350,20),TILE_TRANSCOLOR)
    win = Window((ALIGN_CENTER,0),winSet,3,SHOP_TITLE,None,[btn])
    
    statLabel = Label("Str:",WIN_FONT,WIN_FONT_COLOR,(20,20))
    statWin = Window((ALIGN_CENTER,340),winSet,0,None,[statLabel])
