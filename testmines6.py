...
def game(screen):
    ...
    while True:
        ...
        for zombie in zombies: 
            #if the zombie isn't busy, let the AI kick in to choose a direction,then try and act in that direction
            if(not zombieAct):
                nearTiles=tilemap.getNearTiles(zombie.getPos())
                newDir=zombie.runAI(nearTiles)
                zTryResult = tryAction(screen,zombie,newDir,tilemap)
                if(zTryResult):
                    zActResult=zTryResult
